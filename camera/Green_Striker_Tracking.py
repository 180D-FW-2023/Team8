...
while(1):
    ...
    Extracted_Striker_Frame = cv.imread('ExtractedStriker.png')
    ...
    if len(good)>MIN_MATCH_COUNT:
        ...
        h,w, _ = frame_Gray.shape
        pts = np.float32([ [0,0],[0,h-1],[w-1,h-1],[w-1,0] ]).reshape(-1,1,2)
        dst = cv.perspectiveTransform(pts,M)
        Extracted_Striker_Frame_Gray = cv.polylines(Extracted_Striker_Frame_Gray,[np.int32(dst)],True,255,3, cv.LINE_AA)
    else:
    ...
    draw_params = dict(matchColor = (0,255,0), # draw matches in green color
                   ...
                   flags = 2)
    img3 = cv.drawMatches(frame_Gray,kp1,Extracted_Striker_Frame_Gray,kp2,good,None,**draw_params)
